...
const FilterForm = ({ categories, onCategoryChange }) => {
  const form = useForm({
  ...
  const handleCheckboxChange = useCallback(
    (checked, categoryValue, field) => {
      let updatedValues = checked
        ? [...field.value, categoryValue]
        : field.value.filter((value) => value !== categoryValue);
      ...
      if (updatedValues.includes("all")) {
        updatedValues = updatedValues.filter((v) => v !== "all");
      }
...
const ResourcesResult = ({ posts, categories }) => {
...
const ResourcesCard = ({ category, title, thumbnail, summary, link, cta }) => {
  return (
